...
template <int VertexCount, typename MatrixVector, typename IndexVector, typename VertexGetter>
void appendLowerTriangleBlocks(
    const MatrixVector& matrices,
    const IndexVector& indices,
    const vector<int>& boundaryTypes,
    std::vector<Triplet<double>>& coefficients,
    VertexGetter getVertex)
{
    const size_t blockCount = (std::min)(matrices.size(), indices.size());
    std::vector<size_t> offsets(blockCount + 1, 0);

    tbb::parallel_for(size_t(0), blockCount, [&](size_t block) {
        size_t count = 0;
        for (int localRowVertex = 0; localRowVertex < VertexCount; ++localRowVertex) {
            ...
            for (int localColumnVertex = 0; localColumnVertex < VertexCount; ++localColumnVertex) {
                ...
                for (int rowDimension = 0; rowDimension < 3; ++rowDimension) {
                    const int globalRow = 3 * rowVertex + rowDimension;
                    for (int columnDimension = 0; columnDimension < 3; ++columnDimension) {
                        const int globalColumn = 3 * columnVertex + columnDimension;
                        const double value = matrices[block](
                            3 * localRowVertex + rowDimension,
                            3 * localColumnVertex + columnDimension);
                        if (globalRow >= globalColumn && value != 0.0) {
                            ++count;
                        }
                    }
                }
    ...
    const size_t outputBase = coefficients.size();
    coefficients.resize(outputBase + offsets.back());
    tbb::parallel_for(size_t(0), blockCount, [&](size_t block) {
        size_t output = outputBase + offsets[block];
        for (int localRowVertex = 0; localRowVertex < VertexCount; ++localRowVertex) {
            const int rowVertex = getVertex(indices[block], localRowVertex);
            if (rowVertex < 0
                || static_cast<size_t>(rowVertex) >= boundaryTypes.size()
                ...
                for (int rowDimension = 0; rowDimension < 3; ++rowDimension) {
                    ...
                    for (int columnDimension = 0; columnDimension < 3; ++columnDimension) {
                        ...
                        const double value = matrices[block](
                            ...
                            3 * localColumnVertex + columnDimension);
                        if (globalRow >= globalColumn && value != 0.0) {
                            coefficients[output++] = Triplet<double>(globalRow, globalColumn, value);
                        }
                    }
                }
            }
...
std::vector<Eigen::Triplet<double>> BHessian::toTriplets(const vector<int>& Btype) const
{
    ...
    toTriplets(Btype, coefficients);
    return coefficients;
}

void BHessian::toTriplets(
    const vector<int>& Btype,
    std::vector<Eigen::Triplet<double>>& coefficients) const {
    coefficients.clear();
    appendLowerTriangleBlocks<1>(
        H3x3, D1Index, Btype, coefficients,
        [](int vertex, int) { return vertex; });
    appendLowerTriangleBlocks<2>(
        H6x6, D2Index, Btype, coefficients,
        [](const Vector2i& vertices, int local) { return vertices[local]; });
    appendLowerTriangleBlocks<3>(
        H9x9, D3Index, Btype, coefficients,
        [](const Vector3i& vertices, int local) { return vertices[local]; });
    appendLowerTriangleBlocks<4>(
        H12x12, D4Index, Btype, coefficients,
        [](const Vector4i& vertices, int local) { return vertices[local]; });
    return;
...
double SelfConstraintVal(const mesh3D& mesh, const EncodedContact& active) {
    double val;
    if (active[0] >= 0) {
        // edge-edge
...
void compute_g_dpt_new(const mesh3D& mesh,
    const std::vector<EncodedContact>& activeSet,
    ...
    double coef, double d_hat)
{
    for (int ii = 0; ii < activeSet.size(); ii++) {
        ...
        if (MMCVIDI[0] >= 0) {
            // edge-edge
            Eigen::Vector3d v0 = mesh.vertexes[MMCVIDI[1]] - mesh.vertexes[MMCVIDI[0]];
            Eigen::Vector3d v1 = mesh.vertexes[MMCVIDI[2]] - mesh.vertexes[MMCVIDI[0]];
            Eigen::Vector3d v2 = mesh.vertexes[MMCVIDI[3]] - mesh.vertexes[MMCVIDI[0]];
            Eigen::Matrix3d Ds;
            Ds.col(0) = v0;
            Ds.col(1) = v1;
            Ds.col(2) = v2;
            ...
            if (MMCVIDI[2] < 0) {
                ...
                Eigen::Vector3d v0 = mesh.vertexes[MMCVIDI[1]] - mesh.vertexes[v0I];
                ...
                Eigen::Vector3d v0 = mesh.vertexes[MMCVIDI[1]] - mesh.vertexes[v0I];
                Eigen::Vector3d v1 = mesh.vertexes[MMCVIDI[2]] - mesh.vertexes[v0I];
                ...
                auto rotate_uv0 = rotation * pos0;
                auto rotate_uv1 = rotation * mesh.vertexes[MMCVIDI[1]];
                auto rotate_uv2 = rotation * mesh.vertexes[MMCVIDI[2]];
                ...
                Eigen::Vector3d v0 = mesh.vertexes[MMCVIDI[1]] - mesh.vertexes[v0I];
                Eigen::Vector3d v1 = mesh.vertexes[MMCVIDI[2]] - mesh.vertexes[v0I];
                Eigen::Vector3d v2 = mesh.vertexes[MMCVIDI[3]] - mesh.vertexes[v0I];
                Eigen::Matrix3d Ds;
                Ds.col(0) = v0;
                Ds.col(1) = v1;
                Ds.col(2) = v2;
                Eigen::Vector3d normal = (mesh.vertexes[MMCVIDI[2]] - mesh.vertexes[MMCVIDI[1]]).cross(
                    mesh.vertexes[MMCVIDI[3]] - mesh.vertexes[MMCVIDI[1]]).normalized();
                ...
                Eigen::Vector3d u2 = mesh.vertexes[MMCVIDI[3]] - pos0;
                Eigen::Matrix3d Dm;
                Dm.col(0) = u0;
                Dm.col(1) = u1;
                Dm.col(2) = u2;
    ...
    vector<Vector3d>& grad_inc, double dHat, double coef)
{
    ...
    for (int cI = 0; cI < e_db_div_dd.size(); ++cI) {
        ...
        const EncodedContact& MMCVIDI = mesh.Self_EE_ActiveSet[cI];
        double eps_x, e;
...
void Self_largestFeasibleStepSize(
    const mesh3D& mesh,
    const vector<Eigen::Vector3d>& searchDir,
    double slackness,
    double& stepSize)
{
    const auto& constraintSet = mesh.Self_CCD_ActiveSet;
    if (constraintSet.empty()) {
        return;
    }

    const double ccdDistanceRatio = 1.0 - slackness;
    Eigen::VectorXd largestAlphas(constraintSet.size());
    tbb::parallel_for(0, static_cast<int>(constraintSet.size()), 1, [&](int constraint) {
        const auto& pair = constraintSet[constraint];
        if (pair.first < 0) {
            const int surfaceVertex = -pair.first - 1;
            const int point = static_cast<int>(mesh.surfVerts[surfaceVertex]);
            const Vector4i& face = mesh.surface[pair.second];
            largestAlphas[constraint] = point_triangle_ccd(
                mesh.vertexes[point],
                mesh.vertexes[face[0]],
                mesh.vertexes[face[1]],
                mesh.vertexes[face[2]],
                -searchDir[point],
                -searchDir[face[0]],
                -searchDir[face[1]],
                -searchDir[face[2]],
            ...
            const auto& firstEdge = mesh.surfEdges[pair.first];
            const auto& secondEdge = mesh.surfEdges[pair.second];
            largestAlphas[constraint] = edge_edge_ccd(
                mesh.vertexes[firstEdge.first],
                mesh.vertexes[firstEdge.second],
                mesh.vertexes[secondEdge.first],
                mesh.vertexes[secondEdge.second],
                -searchDir[firstEdge.first],
                -searchDir[firstEdge.second],
                -searchDir[secondEdge.first],
                -searchDir[secondEdge.second],
                ccdDistanceRatio,
                0.0);
        }
    });
    stepSize = (std::min)(stepSize, largestAlphas.minCoeff());
}
    ...
    double& stepSize)
{
    ...
    const double pointTriangleSweepStepSize = stepSize;
    ...
    tbb::parallel_for(0, (int)mesh.surfVerts.size(), 1, [&](int svI)
        {
            ...
            const CPUAABB pointBox = makeSweptPointAABB(
                mesh.vertexes[vI], searchDir[vI], pointTriangleSweepStepSize);
            // triangle-point
            for (const auto& sfI : triInds)
            {
                ...
                const CPUAABB faceBox = makeSweptTriangleAABB(
                    ...
                    { searchDir[sfVInd[0]], searchDir[sfVInd[1]], searchDir[sfVInd[2]] },
                    pointTriangleSweepStepSize);
                if (!pointBox.overlaps(faceBox)) {
                    continue;
                }
    ...
    // PT has already established a safe upper bound. Restrict EE broad-phase
    // filtering to that interval; ACCD still receives the full search
    // direction and therefore returns alpha in the original global [0, 1]
    // parameterization.
    const double edgeEdgeSweepStepSize = stepSize;
    ...
    tbb::parallel_for(0, (int)mesh.surfEdges.size(), 1, [&](int eJ)
        {
            ...
            for (const auto& eI : edgeInds) {
                ...
                const CPUAABB secondEdgeBox = makeSweptEdgeAABB(
                    ...
                    edgeEdgeSweepStepSize);
                if (!firstEdgeBox.overlaps(secondEdgeBox)) {
                    continue;
                }
