    ...
    double energy = parallel_reduce(
            ...
            [&](const tbb::blocked_range<int> &rg, double temp_sum) {
                for (int i = rg.begin(); i != rg.end(); i++) {
                    ...
                    RotationAwareSVDResult svdResult = computeRotationAwareSVD(F);
                    Matrix3d V, S, sigma;
                    sigma = svdResult.singularValueMatrix;


                    V = svdResult.rightSingularVectors;
...
TetPFPX computePFPX3D_double(const Matrix3d &InverseDm) {
    TetPFPX PFPX = TetPFPX::Zero();
...
TrianglePFPX computePFPX32D_double(const Matrix2d &InverseDm) {
...
void initMesh3D(mesh3D &mesh, int type, double scale) {
    ...
    mesh.DM_tetrahedra_inverse.reserve(mesh.tetrahedraNum);
    mesh.tetrahedraPFPX.reserve(mesh.tetrahedraNum);
    mesh.volum.reserve(mesh.tetrahedraNum);
    mesh.DM_triangle_inverse.reserve(mesh.triangleNum);
    mesh.trianglePFPX.reserve(mesh.triangleNum);
    mesh.areas.reserve(mesh.triangleNum);
...
//        mesh.DM_tetrahedra_inverse.push_back(DMInverse);
        ...
        mesh.DM_tetrahedra_inverse.push_back(DMInverse);
        mesh.tetrahedraPFPX.push_back(computePFPX3D_double(DMInverse));
...
//
        ...
        mesh.DM_triangle_inverse.push_back(DMInverse);
        mesh.trianglePFPX.push_back(computePFPX32D_double(DMInverse));
...
MatrixXd project_StabbleNHK_H_3D(const Matrix3d &F, const double &lengthRate, const double &volumRate) {
    ...
    U = svdResult.leftSingularVectors;
    sigma = svdResult.singularValueMatrix;
    V = svdResult.rightSingularVectors;
    ...
    double lamda[9];
    vector<double> roots = ElasticityMath::solveCubicRealRoots(
        aa, bb, cc, dd, 1e-1);
    ...
    for (int i = 0; i < 9; i++) {
        if (lamda[i] > 0) {
            H += lamda[i] * ElasticityMath::vectorizeColumnMajor(Q[i])
                * ElasticityMath::vectorizeColumnMajor(Q[i]).transpose();
        }
    }
    return H;
}

Eigen::Matrix<double, 9, 9> project_StabbleNHK_2_H_3D(
    const Matrix3d &F, const double &lengthRate, const double &volumRate) {
    ...
    if (false) {
        ...
        vector<double> roots = ElasticityMath::solveStableNeoHookeanCubic(
            1, c2, c1, c0);
...
project_ANIOSI5_H_3D(const Matrix3d &F, Vector3d direction, const double &scale, const double &contract_length) {
    ...
    MatrixXd H = lamda0 * ElasticityMath::vectorizeColumnMajor(Q0)
        * ElasticityMath::vectorizeColumnMajor(Q0).transpose();
    if (lamda1 > 0) {
        H += lamda1 * ElasticityMath::vectorizeColumnMajor(Q1)
            * ElasticityMath::vectorizeColumnMajor(Q1).transpose();
        H += lamda2 * ElasticityMath::vectorizeColumnMajor(Q2)
            * ElasticityMath::vectorizeColumnMajor(Q2).transpose();
    }
