...
public: // constructor
    ...
    void locateVoxelAxisIndex(const Eigen::Vector3d& pos, Eigen::Array<int, 1, 3>& voxelAxisIndex);
    int locateVoxelIndex(const Eigen::Vector3d& pos);
    int voxelAxisIndex2VoxelIndex(const int voxelAxisIndex[3]);
    int voxelAxisIndex2VoxelIndex(int ix, int iy, int iz);
    void queryPointForTriangles(const Eigen::Vector3d& pos, double radius, std::unordered_set<int>& triInds);
