...
- **线性求解器结构**：矩阵装配、RHS、解向量和后端分派已移入 `NewtonLinearSystem`；`LinearSolverOptions` 在 PARDISO 可用时首选 PARDISO，否则回退块感知 SuiteSparse LDL；CHOLMOD/METIS 与 Eigen-CG 继续通过 `--linear-solver` 正式保留。旧自定义 PCG、多 RHS 实验入口及 `mesh3D::Constraints` 已删除。
