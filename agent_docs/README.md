...
3. `SimulationScene` 已接通四个入口：默认 `ClothOverBunny`、hard `TwistingMat`、使用相同网格/目标的 soft `TwistingMatSoft` 范例，以及参考 GPU_IPC 的双 `bunny2` 大场景（每只 scale=0.2）。场景参数在 `Assets/scene/parameterSetting.txt`，现按键名解析并校验未知、重复、缺失及越界值。
